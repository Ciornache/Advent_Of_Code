...
public:
    ...
    void traverseGrid(int startRow, int startColumn, int endRow, int endColumn, bool type)
    {
        //        std::cout << n << ' ' << m << ' ' << maxTime << '\n';
        std::priority_queue<element> pq;
        for (int i = 0; i < maxTime; ++i)
        {
            if (dp[startRow][startColumn][i] == 1e9)
                dp[startRow][startColumn][i] = 0;
            pq.push({startRow, startColumn, i});
            if (type)
                break;
        }
        while (!pq.empty())
    ...
    {
        simulateWind();
        traverseGrid(startRow, startColumn, endRow, endColumn, 1);
    }
    ...
    void solveSecondTask()
    {
        ...
        traverseGrid(endRow, endColumn, startRow, startColumn, 0);
        for (int i = 1; i <= MAX_ROW; ++i)
        {
            for (int j = 1; j <= MAX_COLUMN; ++j)
            {
        ...
        traverseGrid(startRow, startColumn, endRow, endColumn, 0);
        int answer = 1e9;
        for (int i = 0; i <= MAX_TIME; ++i)
            answer = min(answer, dp[endRow][endColumn][i]);
        std::cout << answer << '\n';
    }
};
